...
class AppData {
    ...
    func load() {
        pokemons = storage.load(Constants.pokemonFile, from: directory(), as: [LocalPokemon].self) ?? [LocalPokemon]()
    }
    
    func save() {
        storage.save(pokemons, to: directory(), as: Constants.pokemonFile)
    }
